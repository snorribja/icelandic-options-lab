...
from global_vars import ICELANDIC_MARKET_CLOSED_DATES, CBI_RATE_URL, REIBOR_SERIES

#! mögulega færa adjust date föllinn eh annað og gera ráð fyrir sanatized dateum
#! volatility gæti verið reiknað ekki rétt út ef það er 0 - of lítil saga
...
def get_volatility(stock_name: str, start_date: date, end_date: date, current_date: date = date.today(), price_column: str = "Close") -> float:
    ...
    past_stock_prices = past_stock_info[price_column].to_numpy()
...
def get_annual_risk_free_interest_rate(T: float, t: float, as_of_date: date) -> float:
    ...
    try:
        ...
        response = requests.get(
            "https://indexes.nasdaqomx.com/docs/Fixed_duration_indexYield_1998_to_202207.xlsx",
            timeout=20,
        )
        response.raise_for_status()
        rows = load_workbook(
...
def get_info_for_option_price(stock_name: str, start_date: date, end_date: date, current_date: date = date.today()):
    ...
    volatility = get_volatility(stock_name=stock_name, start_date=start_date, end_date=end_date, current_date=current_date)
